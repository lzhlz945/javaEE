...
public class EahcaheTest {
     ...
     * 			效果：数据会从二级缓存中获取
     * 				查出的数据都会被默认先放在一级缓存中。
     * 				只有会话一提交或者关闭以后，级缓存中的数据才会转移到二级缓存中
     * 		使用：
     ...
     */
    @Test
    public void TestEJhc() throws IOException {

        SqlSessionFactory sqlSession = getSqlSession();
        SqlSession sqlSession1 = sqlSession.openSession();
        SqlSession sqlSession2 = sqlSession.openSession();
        EmployeeMapper1 mapper1 = sqlSession1.getMapper(EmployeeMapper1.class);
        EmployeeMapper1 mapper2 = sqlSession2.getMapper(EmployeeMapper1.class);
        mapper1.getEmps();
        sqlSession1.close();
        System.out.println("二级缓存取数据！！！！");
        mapper2.getEmps();
